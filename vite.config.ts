...
const plugins = [
  ...
  VitePWA({
    // ─── لماذا "prompt" لا "autoUpdate" ────────────────────────────────────
    // كان الإعداد `autoUpdate` بينما ينتظر الكود في `main.tsx` أن يُستدعى
    // `onNeedRefresh` ليعرض زرّ التحديث — و`autoUpdate` لا يستدعيه إطلاقاً.
    // فلا زرّ يظهر، ولا الصفحة المفتوحة تُحمّل الحزمة الجديدة: العامل الجديد
    // يتولّى التحكّم لكن ما في الذاكرة يبقى كما هو حتى يُعاد التحميل. وتبويبةٌ
    // مفتوحة في هاتفٍ تبقى أياماً.
    //
    // والنتيجة رآها صاحب المنتج: أرسل لقطة يشكو من تصميمٍ قديم، ونصُّها لم
    // يعد موجوداً في الحزمة المنشورة أصلاً. نشرٌ لا يصل ليس نشراً.
    //
    // فالآن يُعرض له زرّ «تحديث الآن» — ومع `skipWaiting` أدناه يتولّى العامل
    // الجديد فوراً عند ضغطه. ولا يُعاد التحميل من تلقائه: قد يكون في وسط
    // كتابة رسالة.
    registerType: "prompt",

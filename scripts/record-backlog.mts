...
const BACKLOG: Item[] = [
  ...
  {
    title: "[قرار] نتائج فريق المراجعة تصير مهامّ لها حالة (Autopilot)",
    description: "من تقييم multica. اليوم يبلّغ الفريق كل ٤ ساعات والبلاغ يذهب. المقترح: كل نتيجة جديدة تُفتح مهمّةً وتُقفل تلقائياً حين تختفي، وتبدأ بالحرجة وحدها (أمني/مالي/بوّابات دفع). التكلفة: قائمة مهامّ تكبر فتصير ضجيجاً من نوع آخر. مؤجّل بقرارك.",
    priority: "medium",
  },
  {
    title: "[نسخ] اختبار استعادةٍ حقيقي يحتاج صلاحية CREATE DATABASE",
    description: "فحص النسخة الأسبوعي يفكّ التعمية ويتأكّد من الاكتمال والجداول والصفوف — لكنه ليس استعادة. الاستعادة تحتاج قاعدة مؤقّتة، ومستخدم التطبيق يُردّ بـ1044 Access denied. امنح مستخدماً صلاحية CREATE DATABASE على قاعدة اختبار، أو شغّل الاستعادة بحساب له الصلاحية.",
    priority: "high",
  },
  {
    title: "[أمن] مفاتيح PayPal التجريبية في متغيّرات الإنتاج",
    description: "PAYPAL_LIVE_API_* تحمل بيانات sandbox (sb-…@business.example.com) و PAYPAL_MODE=sandbox. البوّابة محجوبة الآن بالكود، لكن المفاتيح نفسها يجب أن تُستبدل ببيانات إنتاج حقيقية أو تُمسح من .env.",
    priority: "high",
  },
];
